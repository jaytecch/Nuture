...
export const expirationDateRequired = (msg, initialDate) => value => {
  const dateIsValid = (value && value.date instanceof Date)
    || (initialDate && initialDate instanceof Date);
  return !dateIsValid ? msg : VALID;
}
...
//ssn length
const SSN_RE = /^(?!000)(?!666)(?!9)\d{3}([- ]?)(?!00)\d{2}\1(?!0000)\d{4}$/i;
...
// phone
const PHONE_RE = /((\(\d{3}\) ?)|(\d{3}-))?\d{3}-\d{4}/i;
...
export const realZip = message => value => {
  return zipcodes.lookup(value) ? VALID : message;
}
...
export const addressZipFormatNotRequired = message => value => {
  if(value) {
    return ZIP_RE.test(value) ? VALID : message;
  }

  return VALID;
}

const STATE_RE = /^(?:(A[KLRZ]|C[AOT]|D[CE]|FL|GA|HI|I[ADLN]|K[SY]|LA|M[ADEINOST]|N[CDEHJMVY]|O[HKR]|P[AR]|RI|S[CD]|T[NX]|UT|V[AIT]|W[AIVY]))$/i;

export const stateFormatValid = message => value => {
  return value && STATE_RE.test(value) ? VALID : message;
};

// export const validDLNumber = message => value => {
//   return value && STATE_RE.test(value) ? VALID : message;
// };
